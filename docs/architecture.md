...
- Executable selection shares validation across settings and the picker, accepts spaces/Unicode, preserves display casing and deduplicates case-insensitively. A source-pinned Xray patch compares executable names case-insensitively on Windows at connection lookup, including uppercase extensions and legacy lowercase selections. This works when applications start after the VPN; no startup process snapshot or case-variant guessing is used. A stock core is rejected for process routing with an actionable update error.
- Selecting Overwatch or its launcher includes `Overwatch.exe`, `Overwatch Launcher.exe` and `VivoxVoiceService.exe`; selecting either VALORANT binary includes the other game binary. Derived companions are shown selected in the picker and follow the parent selection in both modes. These are narrow compatibility groups, not a claim that every Overwatch installation uses a separate voice executable. Shared launchers (`Battle.net.exe`, `Agent.exe`, Steam, Riot services) and Windows services are not expanded automatically; other observed helper executables can be explicitly selected. Matching remains by executable basename, so another application with the same basename shares that rule.
- Process enumeration prefers CIM `Win32_Process.ExecutablePath`, falls back to `Get-Process` and uses UTF-8 output. Running state is separate from path availability, saved inactive selections and browsed executables with unknown state. Selected and derived rows sort first, case-insensitively; search still filters the entire list.
...
- In `only` mode, unmatched internet traffic goes direct before any domain policy, including profile proxy domains; local ranges stay direct. Sniffing uses `routeOnly: true` so domain inspection can choose a route without replacing the application's destination IP, including QUIC/UDP endpoints.
...
`npm run download:xray` verifies the official assets, verifies a pinned source archive, applies `scripts/patches/xray-windows-process-names.patch`, runs its Go regression tests and builds `xray.exe` for Windows x64. Go 1.26.8, Git and tar are required. CI runs the regression tests natively on Windows; cross-platform builds exercise the normalization tests and cross-compile the Windows binary. Updating Xray requires deliberately updating the version, source commit/hash and reviewing/reapplying the patch. Modified MPL source and its manifest ship in `resources/xray/levik-source`.

## Split tunnel validation on Windows

1. Install a build containing `levik-process-v1`, enable bypass, select two unrelated applications plus Overwatch and save. Confirm the running processes appear once even when saved names are lowercase, unreadable paths do not imply stopped processes, and all selected/derived rows are above unselected rows.
2. Restart the game and voice session after saving/reconnecting, then verify fresh TCP and UDP rule hits for each application with Kill Switch both enabled and disabled. Also start a selected application after the VPN is already connected. Existing sockets/UDP associations cannot be moved seamlessly between egress paths.
3. Correlate voice source ports with `Get-NetUDPEndpoint`/`OwningProcess` and inspect egress/replies on the physical adapter. If a different voice executable owns the socket, select that exact executable too; do not assume `VivoxVoiceService.exe` exists on every installation. Test IPv4 and IPv6, Wi-Fi and Ethernet, and each supported proxy transport.
4. Test `only` mode against a profile-proxied domain: selected game/companions must use VPN, an unselected browser must use direct. Confirm off mode retains ordinary domain routing.

Unit tests and a cross-compiled executable do not establish that a live Overwatch voice session succeeds. Voice can still fail because of upstream port restrictions, NAT/firewall policy, a helper outside the explicit compatibility groups, or Windows socket ownership lookup failures. The pinned core looks up TCP/UDP owners at the routing decision and TUN keeps UDP associations by source endpoint; shared/reused ports and already-open associations require real Windows packet evidence when diagnosing remaining failures.
